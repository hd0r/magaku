...
function Register() {
  ...
  useEffect(() => {
    const checkEmail = async () => {
      try {
        const response = await axios.get('http://localhost:3000/check-email', { params: { email } });
  ...
  useEffect(() => {
    const checkUsername = async () => {
      try {
        const response = await axios.get('http://localhost:3000/check-username', { params: { username } });
